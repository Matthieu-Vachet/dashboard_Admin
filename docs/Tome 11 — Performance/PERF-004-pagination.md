...
La pagination est serveur pour MongoDB et client uniquement sur un snapshot déjà borné. Les métadonnées `page`, `limit`, `total`, `pages` restent cohérentes.
...
`PokemonGo-API-/src/services/pokemon-identity-service.js`, `PokemonGo-API-/src/current-datasets/router.js`.
