...
`PokemonGo-API` et `PokemonGo-Data` ont été audités. Aucune modification n’y était requise : le contrat nécessaire existe déjà dans `PokemonGo-Data` (`stats`, `quickMoves`, `cinematicMoves`, `combat.*`, types, formes, disponibilités Shadow, classes et calendrier GBL). Le Dashboard est le seul dépôt modifié.
...
Le corpus `scripts/fixtures/pvpoke-parity-2026-07-28.json` contient 20 combats capturés le 28 juillet 2026 sur PvPoke, dépôt officiel au commit `5e1e3d971369a47aaf3e7247f50710d80205d570`. Il couvre 0, 1 et 2 boucliers, Shadow, Déguisement, buffs/debuffs et des fast moves de durées différentes.
...
- `npm run test:pvp-parity` rejoue les 20 fixtures contre le snapshot local.
