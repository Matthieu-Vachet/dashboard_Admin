...
Les routes publiques suivent OpenAPI ; les routes privées exigent le secret admin. Les tests couvrent méthodes, paramètres, codes, headers sûrs et refus des routes inconnues.
...
`PokemonGo-API-/test/api.test.js`, `src/app/api/pokemon-api-proxy/route.ts`.
