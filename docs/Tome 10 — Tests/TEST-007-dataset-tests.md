...
# TEST-007 — Tests des datasets

## Autorité

Un dataset est accepté seulement après validation de schéma, comptage, hash, diff et relecture. Le changement nul produit l’état terminal `unchanged`.
...
`PokemonGo-API-/test/current-dataset-models.test.js`, `PokemonGo-API-/test/current-dataset-pipeline.test.js`.
