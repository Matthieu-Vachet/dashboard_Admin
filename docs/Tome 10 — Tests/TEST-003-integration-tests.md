...
# TEST-003 — Tests d’intégration

## Autorité

Les tests d’intégration vérifient route, service, validation, persistance simulée et réponse publique. Ils prouvent notamment qu’un échec provider conserve le snapshot actif.
...
`PokemonGo-API-/test/current-dataset-pipeline.test.js`, `PokemonGo-API-/test/pokemon-identities.test.js`.
