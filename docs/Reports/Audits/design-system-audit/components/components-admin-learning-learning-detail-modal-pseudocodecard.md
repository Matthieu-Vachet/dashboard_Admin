...
- Component ID: `MWI-COMP-101`.
- Location: `Dashboard Admin/src/components/admin/learning/learning-detail-modal.tsx`:151.
- File range: lines 151–200.
...
| lucide-react | Eye, Save, ScrollText | icons |
| @/components/ui/badge | Badge | internal |
| @/components/ui/button | Button | internal |
| @/components/ui/input | Textarea | internal |
| @/types/admin/learning | LearningProgressMutationResult, LearningProgressPatch, RuntimeLearningPseudocode | internal |
...
Unresolved/external JSX tags: `Eye`, `Save`, `ScrollText`.
...
- <UnitCard>
  ...
  - <p>
  - <label>
  - <Textarea id={`${item.id}-answer`} />
  - <div>
    - <span>
    - <Button>
  - <div>
    - <div>
      - <button aria-expanded={correctionVisible}>
        - <Eye />
      - <Badge>
    - <p>
  - <Validation />
...
Unique HTML/React tags: `Badge`, `button`, `Button`, `CardHeader`, `div`, `Eye`, `label`, `MetaBlock`, `p`, `ProgressButton`, `Save`, `ScrollText`, `span`, `Textarea`, `UnitCard`, `Validation`.

## 5. React structure and state management

- Hooks: `useState`.
- Local state initializers: `answer = item.answer || ""`, `correctionVisible = false`.
- Event handlers exposed in JSX: `onChange`, `onClick`, `onSetProgress`.
...
```tsx
{ item, saving, onSetProgress, onNotify }: { item: RuntimeLearningPseudocode; saving: boolean; onSetProgress: (id: string, patch: LearningProgressPatch) => Promise<LearningProgressMutationResult | null>; onNotify: (tone: "success" | "warning" | "error", message: string) => void }
```

Exact local props contract when statically resolvable:

```tsx
{ item: RuntimeLearningPseudocode; saving: boolean; onSetProgress: (id: string, patch: LearningProgressPatch) => Promise<LearningProgressMutationResult | null>; onNotify: (tone: "success" | "warning" | "error", message: string) => void }
```
...
- Boolean properties for Figma: Hover, Disabled, Error, Warning, Success, Empty, Expanded.
- Text properties: `item`, `onNotify`, `onSetProgress`, `saving`.
- Instance swaps: `Eye`, `Save`, `ScrollText`.
...
| Spacing | `gap-2`, `mt-2`, `mt-3`, `mt-4`, `p-0`, `p-3`, `px-3`, `px-4`, `px-5` |
| Sizing | `h-10`, `min-h-10`, `min-h-12`, `min-h-9`, `w-10` |
| Typography | `font-black`, `font-mono`, `font-semibold`, `hover:text-foreground`, `leading-6`, `text-amber-100`, `text-brand`, `text-brand-2`, `text-brand-3`, `text-cyan-100`, `text-danger`, `text-emerald-100`, `text-foreground`, `text-muted`, `text-rose-100`, `text-sm`, `text-violet-100`, `text-warning`, `text-white`, `text-xs`, `tracking-[0.14em]`, `uppercase`, `whitespace-pre-wrap` |
| Radius | `rounded-lg` |
| Borders/strokes | `border`, `border-brand-2/30`, `border-brand-3/25`, `border-brand-3/30`, `border-brand/20`, `border-brand/25`, `border-brand/30`, `border-danger/30`, `border-danger/35`, `border-line`, `border-transparent`, `border-warning/30`, `border-warning/35`, `border-white/10`, `hover:border-danger/50`, `hover:border-line-strong` |
| Shadows/elevation | Not found |
| Opacity | Not found |
| Background | `bg-brand-2/10`, `bg-brand-3/10`, `bg-brand/[0.05]`, `bg-brand/10`, `bg-brand/12`, `bg-danger/10`, `bg-danger/12`, `bg-danger/15`, `bg-transparent`, `bg-warning/10`, `bg-warning/12`, `bg-white/[0.045]`, `bg-white/[0.06]`, `bg-white/[0.075]`, `hover:bg-danger/20`, `hover:bg-white/[0.08]`, `hover:bg-white/[0.11]` |
| Animation | Not found |
...
- `h-10 w-10 p-0`
- `inline-flex items-center gap-2 text-sm font-black text-brand`
- `min-h-10 px-4 text-sm`
- `min-h-12 px-5 text-sm`
- `min-h-9 px-3 text-xs`
- `mt-2 flex flex-wrap items-center justify-between gap-2`
- `mt-2 font-mono`
- `mt-3 rounded-lg border border-brand/20 bg-brand/[0.05] p-3`
- `mt-3 text-sm font-semibold leading-6 text-muted`
- `mt-3 whitespace-pre-wrap font-mono text-sm leading-6 text-muted`
- `mt-4 block text-xs font-black uppercase tracking-[0.14em] text-brand-2`
...
- ARIA attributes: `aria-expanded={correctionVisible}`.
- Roles: Not found.
...
- Lucide icons: `Eye`, `Save`, `ScrollText`.
...
- Boolean properties: Hover, Disabled, Error, Warning, Success, Empty, Expanded.
- Instance swaps: `Eye`, `Save`, `ScrollText`.
