...
- Component ID: `MWI-COMP-108`.
- Location: `Dashboard Admin/src/components/admin/learning/learning-detail-modal.tsx`:331.
- File range: lines 331–341.
...
Unique HTML/React tags: `div`, `h3`, `section`, `span`.
...
| completed | Not found | See exact signature/contract below |
| icon | Not found | See exact signature/contract below |
| title | Not found | See exact signature/contract below |
| total | Not found | See exact signature/contract below |

Exact parameter signature:

```tsx
{ icon, title, completed, total, children }: { icon: ReactNode; title: string; completed: number; total: number; children: ReactNode }
```

Exact local props contract when statically resolvable:

```tsx
{ icon: ReactNode; title: string; completed: number; total: number; children: ReactNode }
```
...
- Text properties: `children`, `completed`, `icon`, `title`, `total`.
...
| Spacing | `gap-2`, `gap-3`, `mt-3`, `p-4`, `sm:p-5` |
| Sizing | Not found |
| Typography | `font-black`, `font-mono`, `text-brand-2`, `text-lg`, `text-muted`, `text-xs` |
| Radius | `rounded-lg` |
...
- `inline-flex items-center gap-2 text-lg font-black`
- `mt-3 grid gap-3`
- `rounded-lg border border-line bg-white/[0.035] p-4 sm:p-5`
- `text-brand-2`
