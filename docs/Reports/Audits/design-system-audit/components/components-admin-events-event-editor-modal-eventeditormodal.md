...
- Component ID: `MWI-COMP-045`.
- Location: `Dashboard Admin/src/components/admin/events/event-editor-modal.jsx`:8.
- File range: lines 8–51.
...
| @/data/pokemon-events | POKEMON_EVENT_TYPES | internal |
| @/components/admin/pokemon/admin-ui | buttonClass, primaryButtonClass | internal |
...
|---|---|---|
| MWI-COMP-047 | [Field](../components/components-admin-events-event-editor-modal-field.md) (MWI-COMP-047) | JSX/import relation |
| MWI-COMP-048 | [SelectField](../components/components-admin-events-event-editor-modal-selectfield.md) (MWI-COMP-048) | JSX/import relation |
| MWI-COMP-049 | [Area](../components/components-admin-events-event-editor-modal-area.md) (MWI-COMP-049) | JSX/import relation |

Unresolved/external JSX tags: `ModalPortal`, `Save`, `X`.
...
Unique HTML/React tags: `Area`, `article`, `button`, `div`, `Field`, `h2`, `ModalPortal`, `p`, `Save`, `SelectField`, `X`.
...
| statusOptions | Not found | See exact signature/contract below |

Exact parameter signature:

```tsx
{ draft, busy, statusOptions, onChange, onClose, onSave }
```
...
- Boolean properties for Figma: Hover, Focused, Disabled, Scrollable.
- Text properties: `busy`, `draft`, `onChange`, `onClose`, `onSave`, `statusOptions`.
- Instance swaps: `Save`, `X`.
...
| Spacing | `gap-2`, `gap-3`, `gap-4`, `inset-0`, `mb-5`, `mt-1`, `mt-3`, `mt-5`, `p-3`, `p-4`, `px-4`, `py-2`, `sm:p-6` |
| Sizing | `h-10`, `max-h-[92vh]`, `max-w-5xl`, `min-h-11`, `min-h-12`, `w-10`, `w-full` |
| Typography | `font-black`, `font-bold`, `placeholder:text-slate-500`, `text-2xl`, `text-cyan-200/70`, `text-sm`, `text-white`, `text-xs`, `tracking-[0.22em]`, `uppercase` |
...
| `max-h-[92vh]` | base | static occurrence |
...
- `max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-white/10 bg-[#07111f] p-4 shadow-[0_30px_120px_rgba(0,0,0,.5)] sm:p-6`
...
- Responsive utilities: `lg:grid-cols-2`, `sm:p-6`.
...
- Boolean properties: Hover, Focused, Disabled, Scrollable.
- Instance swaps: `Save`, `X`.
