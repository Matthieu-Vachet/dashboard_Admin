...
- Component ID: `MWI-COMP-036`.
- Location: `Dashboard Admin/src/components/admin/dashboard/dashboard-home-live.tsx`:536.
- File range: lines 536–566.
...
```tsx
{ items }: { items: Array<{ name: string; entries: number }> }
```

Exact local props contract when statically resolvable:

```tsx
{ items: Array<{ name: string; entries: number }> }
```
...
| Spacing | `gap-2`, `mt-4`, `space-y-2` |
| Sizing | `h-2`, `h-full` |
| Typography | `font-black`, `font-mono`, `text-muted`, `text-right`, `text-xs` |
| Radius | `rounded-full` |
...
| `grid-cols-[3rem_minmax(0,1fr)_4rem]` | base | static occurrence |
...
- `grid grid-cols-[3rem_minmax(0,1fr)_4rem] items-center gap-2 text-xs font-black`
