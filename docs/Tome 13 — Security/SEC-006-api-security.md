...
# SEC-006 — Sécurité API

## Autorité

Validation de schéma, liste blanche route+méthode, rate limiting, timeout, headers sûrs et erreurs sans secret constituent la frontière de l’API Explorer.
...
`src/app/api/pokemon-api-proxy/route.ts`, `src/lib/pokemon-api-private-registry.ts`.
