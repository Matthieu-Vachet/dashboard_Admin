...
Les pages Admin exigent une session Dashboard. Les credentials et secrets restent exclusivement côté serveur et sont refusés en production s’ils sont absents.
...
`src/lib/auth.ts`, `src/app/api/pokemon-api-proxy/route.ts`.
