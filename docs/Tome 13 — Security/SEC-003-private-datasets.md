...
# SEC-003 — Datasets privés

## Autorité

Les datasets privés ne figurent pas dans le contrat OpenAPI public et leurs réponses sont `private, no-store`. Le Dashboard injecte le secret serveur.
...
`PokemonGo-API-/src/current-datasets/router.js`, `src/lib/pokemon-api-private-registry.ts`.
