...
Un alias exact Identity Manager ou un mapping approuvé peut associer Pikachu Willow, les chapeaux Valor/Mystic/Instinct et Mordudor 10e anniversaire. Un libellé inconnu reste `identity-unresolved`, accompagné de candidats si une aide textuelle existe. Lorsqu’aucun candidat n’est proposé mais que la fiche JSON existe, l’action « Rechercher et lier une fiche JSON » interroge le catalogue canonique Identity Manager, limité au même `dexId`, puis crée après confirmation un alias Margxt manuel et historisé. L’interface n’affiche jamais ce cas comme divergence métier et ne modifie aucun JSON Pokémon.
