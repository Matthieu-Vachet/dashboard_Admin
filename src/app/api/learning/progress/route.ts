...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as { itemId?: unknown; status?: unknown; answer?: unknown };
    ...
    if (body.status !== undefined && !["not_started", "in_progress", "completed"].includes(String(body.status))) {
    ...
    const progress = await updateLearningProgress(session.email, {
      itemId,
      status: body.status as LearningStatus | undefined,
      answer: body.answer as string | undefined,
    });
    return learningJson({ success: true, data: { progress } });
  } catch (error) {
