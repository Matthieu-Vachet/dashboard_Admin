      ...
      <article className="rounded-2xl border border-white/10 bg-white/[0.055] p-4 shadow-[0_22px_90px_rgba(0,0,0,.24)] backdrop-blur-xl sm:p-5">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black text-white">Modifications surveillées</h2>
            <p className="mt-2 text-sm font-bold leading-6 text-slate-400">
              Les changements de commits, tags, ETag, Last-Modified et statuts HTTP remontent ici.
            </p>
          </div>
          <button
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.075] px-4 py-2 text-sm font-black text-white transition hover:border-cyan-200/50 hover:bg-cyan-400/15"
            type="button"
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <strong className="block break-words font-black text-white">{item.name || item.sourceId || "Source"}</strong>
                    <small className="mt-1 block text-xs font-bold text-slate-400">{formatDate(item.checkedAt)}</small>
                    {item.message ? (
                      <p className="mt-2 text-sm font-bold leading-6 text-slate-300">{item.message}</p>
                    ) : null}
                  </div>
                  <span className="shrink-0 rounded-full border border-cyan-200/25 bg-slate-950/35 px-3 py-1.5 font-mono text-xs font-black text-cyan-100">
                    {shortValue(item.version || item.signature)}
                  </span>
                </div>
                <div className="mt-3 grid gap-2 text-xs font-bold text-slate-400 sm:grid-cols-2">
                  <span className="truncate rounded-xl border border-white/10 bg-slate-950/35 px-3 py-2">
      ...
      <article className="rounded-2xl border border-white/10 bg-white/[0.055] p-4 shadow-[0_22px_90px_rgba(0,0,0,.24)] backdrop-blur-xl sm:p-5">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-white">Derniers commits</h2>
          </div>
        </div>
        {recentGit.length ? (
          <div className="grid gap-3">
            {recentGit.map((item) => (
              <article className="rounded-2xl border border-white/10 bg-slate-950/35 p-4" key={item.hash || item.message}>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">{item.date || "date inconnue"}</p>
                <strong className="mt-2 block break-words text-sm font-black text-white">{item.message || "Commit"}</strong>
                {item.hash ? (
