...
import { enrichPvpRankingWithLocalData, normalizeSuggestedTeammate, pvpMoveRestriction, pvpTeammatesErrorMessage } from "@/lib/pvp-ranking-local-data.mjs";
...
function MoveBadge({ entry, move, recommended, fastMove, selectedFastMove }) {
  ...
  const restriction = pvpMoveRestriction(entry, move, fastMove);
  return (
    <article className={`rounded-xl border border-line bg-surface-subtle p-3 ${restriction ? "pvp-legacy-move" : ""}`} style={{ borderLeft: `4px solid ${typeColors[String(move?.type || "NORMAL").toUpperCase()] || typeColors.NORMAL}` }}>
      <div className="flex flex-wrap items-start justify-between gap-2"><strong className="text-sm text-domain-foreground">{moveName(move)}</strong><div className="flex flex-wrap gap-1">{recommended ? <span className="rounded-full bg-emerald-400/16 px-2 py-0.5 text-[9px] font-black text-emerald-100">RECOMMANDÉE</span> : null}{restriction ? <span className="rounded-full bg-warning/16 px-2 py-0.5 text-[9px] font-black text-warning-foreground" title={restriction.description}>{restriction.label}</span> : null}</div></div>
...
function PvpDetail({ entry, references, format, onOpenPokemon, onSimulate, onSimulateTeammate, suggestedTeammates, teammatesLoading, teammatesError }) {
  ...
  return (
    <div className="space-y-4 border-t border-line p-3 sm:p-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2"><section className="rounded-2xl border border-line bg-surface-faint p-4"><h3 className="font-black text-domain-foreground">Attaques immédiates</h3><div className="mt-3 space-y-2">{attacksFast.map((move) => <MoveBadge entry={entry} key={move.id} move={move} fastMove recommended={move.id === entry.moveset?.fast} />)}{!attacksFast.length ? <p className="text-sm font-bold text-amber-200">Aucune attaque immédiate locale reliée.</p> : null}</div></section><section className="rounded-2xl border border-line bg-surface-faint p-4"><h3 className="font-black text-domain-foreground">Attaques chargées</h3><div className="mt-3 space-y-2">{attacksCharged.map((move) => <MoveBadge entry={entry} key={move.id} move={move} fastMove={false} selectedFastMove={fastRecommended} recommended={entry.moveset?.charged?.includes(move.id)} />)}{!attacksCharged.length ? <p className="text-sm font-bold text-amber-200">Aucune attaque chargée locale reliée.</p> : null}</div></section></div>
