...
function TemplateCard({ template, onOpen }) {
  return (
    <button className="group min-w-0 rounded-2xl border border-line bg-surface-inset-strong p-3 text-left transition hover:-translate-y-0.5 hover:border-cyan-200/35 hover:bg-cyan-300/[.07]" type="button" onClick={() => onOpen(template.templateId)}>
      ...
      <span className="mt-1 block truncate text-xs font-bold text-muted">{template.categoryLabel || template.category}</span>
...
function LocalComparisonDetail({ item }) {
  return (
    <article className="rounded-2xl border border-line bg-white/[.03] p-3">
      ...
      {item.ambiguousCandidates?.length ? <details className="mt-3 rounded-xl border border-line bg-surface-inset p-3"><summary className="cursor-pointer text-xs font-black text-foreground">Afficher les candidats ambigus</summary><pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-all font-mono text-[10px] text-muted">{JSON.stringify(item.ambiguousCandidates, null, 2)}</pre></details> : null}
    </article>
  );
}
    ...
    <div className="fixed inset-0 z-[95] bg-slate-950/88 p-0 backdrop-blur-lg sm:p-4" role="dialog" aria-modal="true" aria-label="Détail du template Game Master">
      <div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden border-line bg-[#07101f] shadow-2xl sm:rounded-3xl sm:border">
        <header className="flex items-start gap-3 border-b border-line p-4">
          ...
          {template.templateId ? <a className="hidden min-h-10 items-center gap-2 rounded-xl border border-line-medium bg-white/[.05] px-3 text-xs font-black text-foreground sm:inline-flex" href={`/api/pokemon-admin?action=game-master-export&scope=templates&format=json&includeRaw=true&match=exact&q=${encodeURIComponent(template.templateId)}`}><Download size={14} /> Télécharger</a> : null}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-5">
          ...
          {!loading && tab === "history" ? <div className="space-y-2">{history.map((item) => <div className="rounded-2xl border border-line bg-white/[.03] p-3" key={item.snapshotId}><strong className="font-mono text-xs text-domain-foreground">{item.snapshotId}</strong><p className="mt-1 text-xs font-bold text-muted">{formatDate(item.createdAt)} · {formatBytes(item.sizeBytes)} · {item.propertyCount} propriétés</p></div>)}{!history.length ? <p className="text-sm text-muted">Aucun historique.</p> : null}</div> : null}
...
export function GameMasterExplorerPanel() {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-muted"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-200/20 bg-emerald-300/10 px-3 py-1.5 text-emerald-100"><Database size={13} /> MongoDB privé</span><span className="font-mono">Snapshot {currentSnapshot}</span><span>Vérifié {formatDate(summary?.state?.lastCheckedAt)}</span><span>Source {summary?.snapshot?.provider || "PokeMiners"}</span><span>Mise à jour {formatDate(summary?.snapshot?.sourceUpdatedAt)}</span></div>
