...
function SummaryMetric({ icon: Icon, label, value, tone = "cyan" }) {
  ...
  return (
    <div className={`min-w-0 rounded-2xl border p-3 sm:p-4 ${colors[tone]}`}>
      <Icon size={17} />
      <span className="mt-2 block truncate text-[9px] font-black uppercase tracking-[.13em] text-disabled">{label}</span>
...
function Pagination({ meta, onPage }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <span className="font-mono text-xs font-black text-disabled">{Number(meta?.total || 0).toLocaleString("fr-FR")} résultat(s)</span>
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-xs font-black text-foreground-secondary">{page} / {pages}</span>
...
function TemplateCard({ template, onOpen }) {
  return (
    <button className="group min-w-0 rounded-2xl border border-line bg-surface-inset-strong p-3 text-left transition hover:-translate-y-0.5 hover:border-cyan-200/35 hover:bg-cyan-300/[.07]" type="button" onClick={() => onOpen(template.templateId)}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="rounded-full border border-line bg-white/[.05] px-2 py-1 font-mono text-[9px] font-black text-muted">{formatBytes(template.sizeBytes)}</span>
      </div>
      <strong className="mt-3 block break-all font-mono text-xs font-black leading-5 text-domain-foreground group-hover:text-cyan-100">{template.templateId}</strong>
      <span className="mt-1 block truncate type-caption-strong text-muted">{template.categoryLabel || template.category}</span>
      <div className="mt-3 flex flex-wrap gap-1.5">
        <span className="rounded-full border border-line bg-surface-minimal px-2 py-1 text-[9px] font-black text-foreground-secondary">{template.settingType}</span>
        ...
        <span className="rounded-full border border-line bg-surface-minimal px-2 py-1 text-[9px] font-black text-muted">{Number(template.propertyCount || 0)} propriétés</span>
      </div>
    </button>
  );
}
...
function DetailDefinition({ label, value }) {
  return <div className="min-w-0 rounded-xl border border-line-subtle bg-surface-faint p-3"><dt className="text-[9px] font-black uppercase tracking-[.12em] text-disabled">{label}</dt><dd className="mt-1 break-words font-mono text-xs font-bold text-foreground">{value ?? "—"}</dd></div>;
}
...
function LocalComparisonDetail({ item }) {
  return (
    <article className="rounded-2xl border border-line bg-white/[.03] p-3">
      <div className="flex flex-wrap items-center justify-between gap-2"><strong className="font-mono text-xs text-domain-foreground">#{item.pokemonId} {item.pokemon} · {item.form || "normal"}</strong><StatusBadge status={item.mappingStatus} /></div>
      ...
      {item.ambiguityReason ? <div className="mt-3 rounded-xl border border-violet-200/20 bg-violet-300/[.08] p-3"><strong className="text-xs text-violet-100">Raison exacte : {item.ambiguityReason}</strong><p className="mt-1 type-caption-strong text-foreground-secondary">{item.ambiguityExplanation || "Plusieurs identités locales distinctes correspondent."}</p><p className="mt-1 font-mono text-[10px] text-disabled">{item.candidateCount || item.ambiguityCount || 0} candidat(s)</p></div> : null}
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        {[['Normal', item.localAsset?.image, item.localAsset?.source], ['Shiny', item.localAsset?.shinyImage, item.localAsset?.shinySource]].map(([label, image, source]) => <div className="flex items-center gap-3 rounded-xl border border-line-subtle bg-surface-inset-subtle p-2" key={label}>{image ? <img className="h-14 w-14 shrink-0 object-contain" src={image} alt={`Asset local ${label.toLowerCase()}`} /> : <span className="grid h-14 w-14 shrink-0 place-items-center rounded-lg border border-line"><ImageOff className="text-slate-600" size={18} /></span>}<div className="min-w-0"><strong className="text-xs text-domain-foreground">{label}</strong><span className="block break-all font-mono text-[9px] text-disabled">{source || "Asset absent"}</span></div></div>)}
      </div>
      {item.genderVariants?.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{item.genderVariants.map((variant, index) => <div className="flex items-center gap-3 rounded-xl border border-line-subtle bg-white/[.025] p-2" key={`${variant.isFemale}-${variant.image}-${index}`}>{variant.image ? <img className="h-12 w-12 object-contain" src={variant.image} alt={variant.isFemale ? "Asset femelle" : "Asset mâle ou neutre"} /> : <ImageOff className="m-3 text-slate-600" size={18} />}<div><strong className="text-xs text-domain-foreground">{variant.isFemale ? "Femelle" : "Mâle / défaut"}</strong><span className="block break-all font-mono text-[9px] text-disabled">{variant.costume || variant.form || "forme principale"}</span></div></div>)}</div> : null}
      {item.ambiguousCandidates?.length ? <details className="mt-3 rounded-xl border border-line bg-surface-inset p-3"><summary className="cursor-pointer type-label text-foreground">Afficher les candidats ambigus</summary><pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-all font-mono text-[10px] text-muted">{JSON.stringify(item.ambiguousCandidates, null, 2)}</pre></details> : null}
    </article>
  );
}
...
function DatasetRunCard({ run }) {
  ...
  return (
    <article className="rounded-2xl border border-line bg-surface-inset-medium p-3">
      <div className="flex items-center justify-between gap-2">
        <strong className="font-mono text-xs text-domain-foreground">{formatDate(run.startedAt)}</strong>
        <span className={`rounded-full border px-2 py-1 text-[8px] font-black uppercase ${tone}`}>{run.status}</span>
      </div>
      <p className="mt-2 text-[10px] font-bold text-disabled">
...
function PropertiesView({ rows = [] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="font-mono text-[10px] font-black text-disabled">{visible.length.toLocaleString("fr-FR")} / {rows.length.toLocaleString("fr-FR")} propriété(s)</p>
      <div className="space-y-1">{visible.map((row, index) => <div className="group grid gap-1 rounded-xl border border-line-subtle bg-white/[.025] p-2 font-mono text-xs sm:grid-cols-[minmax(0,1fr)_5rem_minmax(0,1fr)_2rem]" key={`${row.path}-${index}`}><button className="break-all text-left text-sky-200" type="button" onClick={() => copyText(row.path, "Chemin copié")}>{row.path}</button><span className="text-violet-200">{row.type}</span><span className="break-all text-foreground-secondary">{row.value}</span><button className="grid h-7 w-7 place-items-center rounded text-disabled hover:bg-surface-emphasis hover:text-domain-foreground" type="button" onClick={() => copyText(row.value, "Valeur copiée")} aria-label={`Copier ${row.path}`}><Copy size={13} /></button></div>)}</div>
    </div>
  );
}
...
function DetailModal({ resource, loading, tab, onTab, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[95] bg-slate-950/88 p-0 backdrop-blur-lg sm:p-4" role="dialog" aria-modal="true" aria-label="Détail du template Game Master">
      <div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden border-line bg-[#07101f] shadow-2xl sm:rounded-3xl sm:border">
        <header className="flex items-start gap-3 border-b border-line p-4">
          ...
          <div className="min-w-0 flex-1"><small className="text-[9px] font-black uppercase tracking-[.15em] text-cyan-200/70">Template Game Master privé</small><h2 className="break-all font-mono text-sm font-black text-domain-foreground sm:text-lg">{template.templateId || "Chargement…"}</h2></div>
          {template.templateId ? <a className="hidden min-h-10 items-center gap-2 rounded-xl border border-line-medium bg-white/[.05] px-3 type-label text-foreground sm:inline-flex" href={`/api/pokemon-admin?action=game-master-export&scope=templates&format=json&includeRaw=true&match=exact&q=${encodeURIComponent(template.templateId)}`}><Download size={14} /> Télécharger</a> : null}
          <button className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-line-medium bg-white/[.05] text-domain-foreground" type="button" onClick={onClose} aria-label="Fermer"><X size={20} /></button>
        </header>
        <div className="flex gap-1.5 overflow-x-auto border-b border-line-subtle p-2" role="tablist">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-5">
          ...
          {!loading && tab === "local" ? <div className="space-y-2">{local.map((item) => <LocalComparisonDetail item={item} key={item.comparisonKey} />)}{!local.length ? <p className="rounded-2xl border border-dashed border-line-medium p-6 text-center text-sm font-bold text-muted">Aucune correspondance Pokémon locale pour ce template.</p> : null}</div> : null}
          {!loading && tab === "assets" ? <div className="grid gap-2 sm:grid-cols-2">{assetRows.map(([label, value]) => <DetailDefinition label={label} value={value} key={label} />)}{local.map((item) => <div className="flex items-center gap-3 rounded-2xl border border-line bg-white/[.03] p-3" key={item.comparisonKey}>{item.localAsset?.image ? <img className="h-16 w-16 object-contain" src={item.localAsset.image} alt="Asset local exact" /> : <span className="grid h-16 w-16 place-items-center rounded-xl border border-line"><ImageOff className="text-slate-600" /></span>}<div className="min-w-0"><strong className="block truncate text-sm text-domain-foreground">Asset local exact</strong><span className="block break-all font-mono text-[10px] text-muted">{item.localAsset?.source || "Absent — aucun fallback normal silencieux"}</span></div></div>)}</div> : null}
          {!loading && tab === "history" ? <div className="space-y-2">{history.map((item) => <div className="rounded-2xl border border-line bg-white/[.03] p-3" key={item.snapshotId}><strong className="font-mono text-xs text-domain-foreground">{item.snapshotId}</strong><p className="mt-1 type-caption-strong text-muted">{formatDate(item.createdAt)} · {formatBytes(item.sizeBytes)} · {item.propertyCount} propriétés</p></div>)}{!history.length ? <p className="text-sm text-muted">Aucun historique.</p> : null}</div> : null}
          {!loading && tab === "diff" ? <div className="space-y-2">{diffs.map((diff) => <div className="rounded-2xl border border-line bg-white/[.03] p-3" key={diff._id || `${diff.snapshotId}-${diff.templateId}`}><div className="flex items-center justify-between gap-2"><strong className="text-sm text-domain-foreground">{diff.changeType}</strong><span className="font-mono text-[10px] text-disabled">{diff.snapshotId}</span></div>{(diff.changes || []).slice(0, 20).map((change, index) => <div className="mt-2 rounded-xl border border-line-subtle bg-surface-inset p-2" key={`${change.path}-${index}`}><p className="break-all font-mono text-[10px] font-black text-cyan-100">{change.type} · {change.path}</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><pre className="max-h-32 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-rose-300/[.05] p-2 font-mono text-[9px] text-rose-100">Avant : {JSON.stringify(change.before, null, 2)}</pre><pre className="max-h-32 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-emerald-300/[.05] p-2 font-mono text-[9px] text-emerald-100">Après : {JSON.stringify(change.after, null, 2)}</pre></div></div>)}</div>)}{!diffs.length ? <p className="text-sm text-muted">Aucune modification historique pour ce template.</p> : null}</div> : null}
        </div>
      </div>
    </div>,
    document.body,
...
export function GameMasterExplorerPanel() {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div className="flex flex-wrap items-center gap-2 type-caption-strong text-muted"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-200/20 bg-emerald-300/10 px-3 py-1.5 text-emerald-100"><Database size={13} /> MongoDB privé</span><span className="font-mono">Snapshot {currentSnapshot}</span><span>Vérifié {formatDate(summary?.state?.lastCheckedAt)}</span><span>Source {summary?.snapshot?.provider || "PokeMiners"}</span><span>Mise à jour {formatDate(summary?.snapshot?.sourceUpdatedAt)}</span></div>
      ...
      {summary?.initialized ? (
        <>
          <div className="flex gap-2 overflow-x-auto rounded-2xl border border-line bg-surface-inset-strong p-2" role="tablist" aria-label="Modes Game Master">
            {modes.map(([id, label, Icon]) => <button className={`flex min-h-11 shrink-0 items-center gap-2 rounded-xl border px-3 text-xs font-black ${mode === id ? "border-cyan-200/40 bg-cyan-300/15 text-domain-foreground" : "border-transparent text-muted hover:bg-white/[.05] hover:text-domain-foreground"}`} type="button" role="tab" aria-selected={mode === id} onClick={() => setMode(id)} key={id}><Icon size={16} /> {label}</button>)}
          </div>

          {["explorer", "search"].includes(mode) ? (
            <div className="space-y-4">
              <section className="grid gap-2 rounded-2xl border border-line bg-surface-inset p-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="Filtres Game Master">
                <label className="relative sm:col-span-2 lg:col-span-4"><Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-disabled" size={17} /><input className={`${fieldClass} pl-12`} value={filters.q} onChange={(event) => setFilter("q", event.target.value)} placeholder="Template ID, Pokémon, forme, valeur, clé…" aria-label="Rechercher dans le Game Master" /></label>
              ...
              {mode === "explorer" && !filters.q && !filters.category && !filters.group && !filters.settingType ? <div className="space-y-4">{categoryGroups.map(([group, groupCategories]) => <section key={group}><h3 className="mb-2 type-overline text-disabled">{group}</h3><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{groupCategories.map((category) => <button className="rounded-2xl border border-line bg-surface-faint p-3 text-left transition hover:border-cyan-200/30 hover:bg-cyan-300/[.07]" type="button" onClick={() => setFilter("category", category.id)} key={category.id}><div className="flex items-center justify-between gap-2"><strong className="text-sm font-black text-domain-foreground">{category.label}</strong><span className="font-mono text-sm font-black text-cyan-100">{category.total}</span></div><p className="mt-2 text-[10px] font-bold text-disabled">+{category.added || 0} · −{category.removed || 0} · ~{category.modified || 0}</p></button>)}</div></section>)}</div> : null}
          ...
          {mode === "comparison" ? (
            <div className="space-y-4">
              ...
              <section className="grid gap-2 rounded-2xl border border-line bg-surface-inset p-3 sm:grid-cols-3">
                <label className="relative sm:col-span-3"><Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-disabled" size={17} /><input className={`${fieldClass} pl-12`} value={comparisonFilters.q} onChange={(event) => setComparisonFilter("q", event.target.value)} placeholder="Pokémon, forme, costume, bundle…" aria-label="Rechercher dans la comparaison locale" /></label>
              ...
              {contentLoading ? <div className="h-52 animate-pulse rounded-2xl bg-surface-minimal" /> : <div className="grid gap-2 md:grid-cols-2">{(comparison?.data || []).map((item) => <article className="rounded-2xl border border-line bg-surface-inset-strong p-3" key={item.comparisonKey}><div className="flex items-start gap-3">{item.localAsset?.image ? <img className="h-14 w-14 shrink-0 rounded-xl border border-line object-contain p-1" src={item.localAsset.image} alt="Asset local exact" /> : <span className="grid h-14 w-14 shrink-0 place-items-center rounded-xl border border-line"><ImageOff className="text-slate-600" size={20} /></span>}<div className="min-w-0 flex-1"><strong className="block truncate text-sm font-black text-domain-foreground">#{item.pokemonId} {item.pokemon}</strong><span className="block truncate font-mono text-[10px] text-disabled">{item.form || "normal"} → {item.localForm || "—"}</span><div className="mt-2 flex flex-wrap gap-1.5"><StatusBadge status={item.mappingStatus} />{item.variantCategory ? <span className="rounded-full border border-line px-2 py-1 text-[9px] font-black text-muted">{item.variantCategory}</span> : null}</div></div></div><div className="mt-3 rounded-xl border border-line-subtle bg-white/[.025] p-2 font-mono text-[10px] text-muted"><strong className="text-foreground-secondary">Asset bundle : </strong>{item.assetBundleValue || item.assetBundleSuffix || "Non fourni par le Game Master"}{item.ambiguityReason ? <p className="mt-2 text-violet-200">Ambiguïté : {item.ambiguityReason} — {item.ambiguityExplanation}</p> : null}</div><button className="mt-2 w-full rounded-xl border border-line bg-surface-faint px-3 py-2 text-left font-mono text-[10px] text-muted hover:text-domain-foreground" type="button" onClick={() => openTemplate(item.templateId)}>{item.templateId}</button></article>)}</div>}
          ...
          {mode === "history" ? (
            <div className="grid gap-4 lg:grid-cols-[minmax(0,.85fr)_minmax(0,1.4fr)]">
              <section className="space-y-4"><div className="space-y-2"><h3 className="text-sm font-black text-domain-foreground">Exécutions</h3>{(runs?.data || []).map((run) => <DatasetRunCard run={run} key={run._id || run.startedAt} />)}</div><div className="space-y-2"><h3 className="text-sm font-black text-domain-foreground">Snapshots</h3>{(snapshots?.data || []).map((snapshot) => <button className={`w-full rounded-2xl border p-3 text-left ${diffFilters.snapshotId === snapshot.snapshotId || snapshot.current && !diffFilters.snapshotId ? "border-cyan-200/30 bg-cyan-300/[.08]" : "border-line bg-surface-inset-medium"}`} type="button" onClick={() => setDiffFilters((current) => ({ ...current, snapshotId: snapshot.snapshotId, page: 1 }))} key={snapshot.snapshotId}><div className="flex items-center justify-between gap-2"><strong className="truncate font-mono text-xs text-domain-foreground">{snapshot.snapshotId}</strong>{snapshot.current ? <span className="rounded-full bg-emerald-300/12 px-2 py-1 text-[8px] font-black uppercase text-emerald-100">Actif</span> : null}</div><p className="mt-2 text-[10px] font-bold text-disabled">{formatDate(snapshot.indexedAt)} · {snapshot.totalTemplates} templates</p><p className="mt-1 text-[10px] font-bold text-muted">+{snapshot.changes?.added || 0} · −{snapshot.changes?.removed || 0} · ~{snapshot.changes?.modified || 0}</p></button>)}</div></section>
              <section className="space-y-3"><div className="grid grid-cols-2 gap-2"><Select aria-label="Type de changement" className={fieldClass} value={diffFilters.type} onChange={(event) => setDiffFilters((current) => ({ ...current, type: event.target.value, page: 1 }))}><option value="">Tous les changements</option><option value="added">Ajoutés</option><option value="removed">Retirés</option><option value="modified">Modifiés</option></Select><Select aria-label="Catégorie de changement" className={fieldClass} value={diffFilters.category} onChange={(event) => setDiffFilters((current) => ({ ...current, category: event.target.value, page: 1 }))}><option value="">Toutes les catégories</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.label}</option>)}</Select></div>{contentLoading ? <div className="h-52 animate-pulse rounded-2xl bg-surface-minimal" /> : (diffs?.data || []).map((diff) => <button className="w-full rounded-2xl border border-line bg-surface-inset-strong p-3 text-left hover:border-cyan-200/30" type="button" onClick={() => openTemplate(diff.templateId)} key={diff._id || `${diff.snapshotId}-${diff.templateId}`}><div className="flex items-center justify-between gap-2"><strong className="break-all font-mono text-xs text-domain-foreground">{diff.templateId}</strong><span className="rounded-full border border-line px-2 py-1 text-[9px] font-black uppercase text-foreground-secondary">{diff.changeType}</span></div><p className="mt-2 text-[10px] font-bold text-disabled">{diff.category} · {(diff.changes || []).length} changement(s){diff.truncated ? " · tronqué" : ""}</p></button>)}<Pagination meta={diffs?.meta} onPage={(page) => setDiffFilters((current) => ({ ...current, page }))} /></section>
            </div>
          ) : null}
        </>
      ) : null}
