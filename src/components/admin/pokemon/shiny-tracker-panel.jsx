...
function Trend({ value, variation }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 font-mono text-xs font-black ${positive ? "text-emerald-300" : negative ? "text-red-300" : "text-muted"}`}>
...
function ShinyDetail({ entry, history, onOpenPokemon }) {
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-4 rounded-2xl border border-line p-4 sm:grid-cols-[9rem_minmax(0,1fr)]" style={typeSurface(entry, 0.18)}>
        <div className="grid min-h-36 place-items-center rounded-2xl border border-line bg-black/25 p-3"><PokemonArtwork pokemon={entry.pokemon} shiny alt={pokemonName(entry)} className="h-32 w-32 border-0 bg-transparent" priority /></div>
      ...
      <section className="rounded-2xl border border-line bg-surface-inset p-4">
        <h3 className="font-black text-domain-foreground">Historique de nos snapshots</h3>
        {statistics ? (
      ...
      {order.map((entry) => {
        ...
        return (
          <button className={`relative flex min-h-[11rem] flex-col items-center justify-end overflow-hidden rounded-2xl border p-3 text-center sm:min-h-[14rem] sm:p-4 ${tone}`} style={typeSurface(entry, 0.15)} key={`${place}-${entry.sourceIdentity?.variantKey || entry.sourceIdentity?.id}`} type="button" onClick={() => onOpen(entry)}>
            ...
            <span className="mt-3 rounded-xl border border-line bg-black/25 px-3 py-2 font-mono text-lg font-black text-domain-foreground">{typeof metric === "number" ? formatNumber(metric, true) : metric || "—"}</span>
        ...
        {rankedEntries.map((entry) => (
          <button className="grid w-full min-w-0 grid-cols-[2rem_3.5rem_minmax(0,1fr)] gap-2 rounded-2xl border border-line p-3 text-left transition hover:border-cyan-200/35 sm:grid-cols-[3rem_4rem_minmax(0,1fr)_repeat(4,minmax(5rem,auto))] sm:items-center sm:gap-3" style={typeSurface(entry, 0.1)} type="button" key={`${entry.rank}-${entry.sourceIdentity?.variantKey}`} onClick={() => openEntry(entry)}>
            ...
            <span className="col-span-3 grid grid-cols-4 gap-2 border-t border-line pt-2 sm:hidden">
              ...
              <span><small className="block text-[9px] font-black uppercase text-disabled">Total</small><strong className="font-mono text-xs text-domain-foreground">{formatNumber(entry.stats?.allTime, true)}</strong></span>
              <span><small className="block text-[9px] font-black uppercase text-disabled">Odds</small><strong className="font-mono text-xs text-amber-100">{entry.shiny?.odds?.raw || "—"}</strong></span>
            </span>
            <span className="hidden sm:block"><small className="block text-[9px] font-black uppercase text-disabled">Daily</small><strong className="font-mono text-sm text-domain-foreground">{formatNumber(entry.stats?.daily, true)}</strong></span>
            <span className="hidden sm:block"><small className="block text-[9px] font-black uppercase text-disabled">Monthly</small><strong className="font-mono text-sm text-domain-foreground">{formatNumber(entry.stats?.monthly, true)}</strong></span>
            <span className="hidden sm:block"><small className="block text-[9px] font-black uppercase text-disabled">Total</small><strong className="font-mono text-sm text-domain-foreground">{formatNumber(entry.stats?.allTime, true)}</strong></span>
