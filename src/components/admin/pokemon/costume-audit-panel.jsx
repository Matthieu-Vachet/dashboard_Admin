  ...
  return (
    <div className="min-w-0 space-y-5">
      <Panel eyebrow="Privé · Margxt comparé à PokemonGo-Data" title="Costumes / Event Pokémon" action={<div className="flex flex-wrap gap-2"><Button icon={<Download size={16} />} disabled={!dataset} onClick={() => downloadJson(dataset)}>JSON privé</Button><Button icon={<RefreshCcw size={16} />} loading={loading} loadingText="Actualisation…" onClick={() => void load()}>Actualiser</Button><Button variant="primary" icon={<RotateCcw size={16} />} loading={regenerating} loadingText="Régénération…" onClick={() => void regenerate()}>Régénérer</Button></div>}>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-4">{[["Référence Margxt", metadata.total], ["Présents localement", metadata.statusCounts?.present], ["À corriger", metadata.statusCounts?.missing], ["Shiny source", metadata.statusCounts?.shinyAvailable]].map(([label, value]) => <div className="rounded-xl border border-line bg-surface-faint p-3" key={label}><span className="text-[10px] font-black uppercase tracking-wider text-muted">{label}</span><strong className="mt-1 block text-2xl">{Number(value || 0).toLocaleString("fr-FR")}</strong></div>)}</div>
        <p className="mt-4 text-sm font-bold leading-6 text-foreground-secondary">Les images Margxt sont conservées comme preuve source seulement. L’interface affiche exclusivement l’asset exact issu de PokemonGo-Data ; toute absence reste visible et produit un diagnostic Identity Manager actionnable.</p>
      ...
      {error ? <ErrorState title="Audit costumes indisponible" description={error} action={<Button onClick={() => void load()}>Réessayer</Button>} /> : null}
