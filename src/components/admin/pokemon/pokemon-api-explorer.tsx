  ...
  return (
    <Card className="min-w-0 overflow-hidden p-4">
      ...
      <div className="mt-4 grid min-w-0 gap-3 xl:grid-cols-[minmax(16rem,23rem)_minmax(0,1fr)_auto]">
        <select className="min-h-11 min-w-0 w-full rounded-lg border border-line bg-white/[0.06] px-3 text-sm font-black outline-none" value={selectedId} onChange={(event) => chooseEndpoint(event.target.value)}>{grouped.map(([group, values]) => <optgroup label={group} key={group}>{(values || []).map((endpoint) => <option key={endpoint.id} value={endpoint.id}>{endpoint.method} · {endpoint.label}</option>)}</optgroup>)}</select>
        <input className="min-h-11 min-w-0 w-full rounded-lg border border-line bg-white/[0.06] px-3 font-mono text-sm font-semibold outline-none" value={path} onChange={(event) => setPath(event.target.value)} aria-label="Route à tester" />
        <Button className="w-full xl:w-auto" variant="primary" type="button" icon={<Play size={16} />} onClick={run} disabled={loading}>{loading ? "Test..." : "Lancer"}</Button>
      </div>
