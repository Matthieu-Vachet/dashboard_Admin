              ...
              <ul className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {changedSources.map((source) => (
                  <li className="rounded-xl border border-amber-300/25 bg-surface-faint p-3" key={source.id || source.name || source.url}>
                    ...
                    {source.readableDiff?.length ? <span className="mt-2 block type-caption-strong text-amber-950 dark:text-amber-50">{source.readableDiff[0]}</span> : null}
                  </li>
                ))}
              </ul>
            ) : null}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  {monitoringKind === "changed" && source.readableDiff?.length ? <ul className="mt-3 space-y-1 rounded-xl border border-amber-300/25 bg-amber-400/[.08] p-3 type-caption-strong text-domain-foreground">{source.readableDiff.slice(0, 12).map((line) => <li className="break-words" key={line}>{line}</li>)}</ul> : null}
