...
const statusClasses = {
  past: "border-line bg-white/[0.03] text-muted",
...
function CompetitionCard({ competition }) {
  return <article className={`grid grid-cols-[3rem_minmax(0,1fr)] gap-3 rounded-xl border p-3 ${tierClasses[competition.tier] || tierClasses.custom}`}><span className="grid h-12 w-12 place-items-center rounded-xl border border-line bg-surface-inset">{competition.iconUrl ? <span aria-hidden="true" className="h-8 w-8 bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url(${JSON.stringify(competition.iconUrl)})` }} /> : <span className="font-mono text-xs">GBL</span>}</span><span className="min-w-0"><span className="flex flex-wrap items-baseline gap-2"><strong className="text-domain-foreground">{competition.sourceName}</strong><small className="font-mono font-black text-muted">PC {competition.cpCap}</small></span>{competition.eligibleTypes?.length ? <span className="mt-2 flex flex-wrap gap-1">{competition.eligibleTypes.map((type) => <small className="rounded-full border border-line bg-black/20 px-2 py-0.5 font-black" key={type}>{type}</small>)}</span> : null}{competition.restrictions?.length ? <span className="mt-2 flex flex-wrap gap-1">{competition.restrictions.map((rule) => <small className="rounded-full border border-amber-300/20 bg-amber-400/10 px-2 py-0.5 font-black text-amber-100" key={rule}>{rule}</small>)}</span> : null}</span></article>;
}
