...
function FeaturedPokemonTile({ featured, detailed = false }) {
  ...
  return (
    <article className="min-w-0 rounded-xl border border-line bg-surface-inset-strong p-2.5">
      <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-3">
        <div className="grid grid-cols-2 gap-1.5" aria-label={`Assets de ${name}`}>
          <div className="min-w-0 text-center">
            ...
            <span className="mt-1 block text-[9px] font-black uppercase tracking-wide text-disabled">Normal</span>
          </div>
          <div className="min-w-0 text-center">
            <PokemonArtwork pokemon={featured} shiny alt={`${name} chromatique`} className={assetSize} />
            <span className="mt-1 block text-[9px] font-black uppercase tracking-wide text-disabled">Shiny</span>
          </div>
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-black text-domain-foreground" title={name}>{name}</p>
      ...
      {detailed && !matched ? (
        <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
          <div className="min-w-0 rounded-lg border border-line bg-surface-faint p-2">
            <dt className="font-black uppercase tracking-wide text-disabled">Alias source</dt>
            <dd className="mt-1 break-words font-mono font-bold text-foreground">{featured?.rawAlias || name}</dd>
          </div>
          <div className="min-w-0 rounded-lg border border-line bg-surface-faint p-2">
            <dt className="font-black uppercase tracking-wide text-disabled">Raison exacte</dt>
    ...
    <article className="min-w-0 rounded-2xl border border-line bg-surface-inset p-4">
      ...
      {moves.length ? (
        <section className="mt-3" aria-label="Aperçu des attaques exclusives">
          ...
          {moves.length > 4 ? <p className="mt-1 text-[10px] font-black text-disabled">+ {moves.length - 4} autre(s) attaque(s)</p> : null}
        </section>
      ) : null}
