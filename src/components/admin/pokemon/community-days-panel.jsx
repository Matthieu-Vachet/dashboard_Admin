...
export function CommunityDaysPanel() {
  ...
  return <section className="space-y-5">
    ...
    <section className="grid gap-3 lg:grid-cols-2 2xl:grid-cols-3">{visible.map((item) => <article className="rounded-2xl border border-white/10 bg-slate-950/35 p-4" key={item.id}><div className="flex gap-3"><div className="grid grid-cols-2 gap-2">{(item.featuredPokemon || []).slice(0, 1).flatMap((entry) => [entry.image, entry.shinyImage]).map((image, index) => image ? <img className="h-20 w-20 rounded-xl bg-white/[.04] object-contain p-2" src={image} alt={index ? `${item.title} shiny` : item.title} key={image} loading="lazy" /> : <div className="h-20 w-20 rounded-xl bg-white/[.04]" key={index} />)}</div><div className="min-w-0"><h3 className="font-black text-white">{item.title}</h3><p className="mt-1 text-xs font-bold text-slate-400">{new Date(item.startDate).toLocaleDateString("fr-FR")} · {item.status}</p><p className="mt-2 text-xs font-bold text-cyan-100">{(item.exclusiveMoves || []).map((move) => `${move.move} (${move.pokemon})`).join(" · ") || "Aucune attaque source"}</p></div></div><ul className="mt-3 space-y-1 text-xs font-bold text-slate-300">{(item.bonuses || []).map((bonus) => <li key={bonus}>• {bonus}</li>)}</ul><div className="mt-3 flex items-center justify-between gap-2"><span className={`rounded-full border px-2 py-1 text-[10px] font-black ${item.featuredPokemon?.every((entry) => entry.resolutionStatus === "matched") ? "border-emerald-200/20 text-emerald-100" : "border-amber-200/20 text-amber-100"}`}>{item.featuredPokemon?.every((entry) => entry.resolutionStatus === "matched") ? "Résolution locale complète" : "Résolution à vérifier"}</span><button className={buttonClass} type="button" onClick={() => setInspect({ title: item.title, value: item })}>Détail JSON</button></div></article>)}</section>
    {inspect ? <div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/90 p-3" role="dialog" aria-modal="true"><div className="max-h-[90vh] w-full max-w-5xl overflow-auto rounded-3xl border border-white/10 bg-[#07101f] p-5"><div className="flex items-center justify-between"><h3 className="text-xl font-black text-white">{inspect.title}</h3><button className={buttonClass} type="button" onClick={() => setInspect(null)}>Fermer</button></div><pre className="mt-4 overflow-auto whitespace-pre-wrap rounded-2xl bg-slate-950/70 p-4 text-xs text-cyan-50">{JSON.stringify(inspect.value, null, 2)}</pre></div></div> : null}
  </section>;
}
