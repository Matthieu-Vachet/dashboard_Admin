      ...
      <section className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" aria-label="Classement Best Defenders">
        {entries.map((entry) => (
          <article className="min-w-0 rounded-2xl border border-line p-4" style={typeSurface(entry)} key={`${entry.tier}-${entry.rank}-${entry.source?.slug}`}>
            <div className="flex items-start justify-between gap-3"><span className="rounded-full border border-emerald-200/25 bg-emerald-400/12 px-3 py-1 text-xs font-black text-emerald-100">Tier {entry.tier}</span><span className="font-mono text-sm font-black text-muted">#{entry.rank}</span></div>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-line bg-black/25 px-3 py-2"><span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-muted"><Shield size={14} /> Résistance</span><strong className="font-mono text-lg text-domain-foreground">{entry.scoreLabel || entry.score?.toLocaleString("fr-FR") || "—"}</strong></div>
            {entry.pokemon?.unmatched ? <p className="mt-3 text-xs font-bold text-amber-200">Identité à résoudre · aucun asset de remplacement.</p> : null}
          </article>
        ))}
      </section>
